...
public class Calc1Controller {
  ...
  @PostMapping
  public String displayResult(@Validated Calc1Form form, BindingResult result, Model model) {
    if (result.hasErrors()) {
      return form(model);
    }
    double calcResult = calc1Service.calculate(form.getDeck(), form.getTarget(), form.getDraw(), form.getWant());
